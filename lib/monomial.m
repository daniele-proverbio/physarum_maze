...
% lightning always chooses a specific single path, using a monomial
% threshold function.
%
% Author: David Palma, PhD, DE <david.palma@uniud.it>
%
% Usage:
%   [v,u,obj] = monomial(B,d,delta,T,debug,r,c)
%
% Arguments:
%   B:     incidence matrix
...
function [v, u, obj] = monomial(B, d, delta, T, debug, r, c)
    ...
    u = double(zeros(size(B, 2), 1)); % arc tensions
    tau = 1.260e-3; % sampling time (tau << 1)
...
function u = sat(y, epsilon)
    ...
    coeff = 20; % coefficient
    gamma = 0.5 + epsilon; % air dielectric rigidity

    %% Monomial threshold function
    u = (y ./ gamma).^(2 * coeff + 1); % arc tensions
